...
def register():
    try:
        ...
        return render_template('auth.activate') 
    except:
        return render_template('auth/register.html')
...
def confirm():
    try:
        ...
        if request.method == 'POST': 
            ...
            db =  get_db()
            attempt = db.execute('SELECT * FROM forgotlink where challenge =? AND state=?', (authid, utils.F_ACTIVE)
            ).fetchone()
...
def change():
    try:
        ...
        if request.method == 'GET':  
            ...
            db =  get_db()
            attempt = db.execute("SELECT userid FROM forgotlink WHERE challenge =? AND state=?", (number, utils.F_ACTIVE)
            ).fetchone()
